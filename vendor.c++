...
void showProducts(int index){
    ...
    do{
        for(int index = 0; index < 100; index++)
            cout<<".";
        cout<<"\n1. Add Items to the cart\n2. Go Back to the Categories List\n3. Go Back to the main menu\n4. Exit"<<endl; 
        for(int index = 0; index < 100; index++)
            cout<<".";    
        cout<<"\nEnter Your Choice: ";
...
void generateBill(cartNode *head)
{
    ...
    while (current != NULL)
    {
        billFile<< current->product_id << "\t\t" << current->product_name << "\t\t\t\t" << current->quantity << "\t\t" <<(current->quantity * current->sum)<<endl;
        total += current->quantity * current->sum;
